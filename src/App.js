...
//creating a reference of the contract using web3 and pasting the CONTRACT API that we created earlier
  ...
  render(){
    return (
      <div className="App">
        Hello, World!
        <div>My Account Address is: {this.state.myAccountAddress}</div>
        <div>Balance {this.state.myAccountBalance} {this.state.tokenSymbol}</div>
        <div>
          ...
          <div>Message: <a>{this.state.msg}</a></div>
        </div>
      </div>
    );
  }
